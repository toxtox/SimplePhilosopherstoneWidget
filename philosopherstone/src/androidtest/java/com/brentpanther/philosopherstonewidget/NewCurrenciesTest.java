...
public class NewCurrenciesTest extends TestCase {
    ...
    public void testProviders() throws InterruptedException {
        ...
        for (PhilosopherstoneExchange phs : values) {
            ...
            Set<String> existingCurrencies = new HashSet<>(Arrays.asList(resources.getStringArray(phs.getCurrencies())));
            for (String currency : currencies) {
                try {
                    String value = phs.getValue(currency);
            ...
            if (!added.isEmpty()) {
                ...
                addedMap.put(phs.getLabel(), TextUtils.join(",", added));
            }
            if (!removed.isEmpty()) {
                Collections.sort(removed);
                removedMap.put(phs.getLabel(), TextUtils.join(",", removed));
            }
        }
